...
function StepperTrigger({
	...
	const handleKeyDown = (e: React.KeyboardEvent<HTMLButtonElement>) => {
		const myIdx = triggerNodes.findIndex(
			(node: HTMLButtonElement) => node === e.currentTarget
		)
		switch (e.key) {
			case "ArrowRight":
			case "ArrowDown":
	...
	return (
		<button
			...
			className={cn(
				"focus-visible:ring-primary/50 focus-visible:border-primary inline-flex cursor-pointer items-center outline-none focus-visible:z-10 focus-visible:ring-3 disabled:pointer-events-none disabled:opacity-60",
...
function StepperIndicator({
	...
	return (
		<div
			...
			data-state={state}
			className={cn(
				"border-border bg-fill1-alpha text-fg-secondary data-[state=completed]:bg-primary data-[state=active]:bg-primary relative flex size-6 shrink-0 items-center justify-center overflow-hidden data-[state=active]:text-white data-[state=completed]:text-white",
...
function StepperSeparator({ className }: React.ComponentProps<"div">) {
	...
	return (
		<div
			...
			data-state={state}
			className={cn(
				"bg-soft-alpha m-0.5 rounded-sm group-data-[orientation=horizontal]/stepper-nav:h-0.5 group-data-[orientation=horizontal]/stepper-nav:flex-1 group-data-[orientation=vertical]/stepper-nav:h-12 group-data-[orientation=vertical]/stepper-nav:w-0.5",
				className
			)}
		/>
	)
