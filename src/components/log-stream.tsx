...
function getStackTrace(metadata: Record<string, unknown>) {
  const topLevelStack = readMetadataString(metadata, "stack");
  if (topLevelStack) return topLevelStack;
...
function LogInspector({ event }: { event: WorkspaceLog }) {
  const [copied, setCopied] = useState(false);
  const stackTrace = getStackTrace(event.metadata);
  const region = readMetadataString(event.metadata, "region") ?? "—";
        ...
        <div className="space-y-6">
          ...
          {event.errorMessage ? (
            <section aria-labelledby="error-heading" className="space-y-3">
              <h3 id="error-heading" className="text-sm font-semibold">
                Error
              </h3>
              <Alert
                color={event.statusCode >= 500 ? "error" : "warning"}
              ...
              >
                ...
                <AlertContent>
                  ...
                  <AlertDescription>
                    Request returned HTTP {event.statusCode} after{" "}
                    {event.durationMs} ms.
                  </AlertDescription>
                </AlertContent>
              </Alert>
          ...
          <section aria-labelledby="metadata-heading">
            <div className="mb-3 flex items-center justify-between gap-3">
              <div>
                <h3 id="metadata-heading" className="text-sm font-semibold">
                  Metadata payload
                </h3>
                <p className="mt-0.5 text-xs text-fg-tertiary">
                  Complete JSON captured for this event
                </p>
              </div>
              <Button
                variant="outline"
...
export function LogStream({
  ...
  const [selectedEvent, setSelectedEvent] = useState<WorkspaceLog | null>(null);
  const [isNavigating, startNavigation] = useTransition();

  const setUrlFilter = useCallback(
    (
      name: "status" | "route" | "q" | "range",
      value: string,
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      setUrlFilter("q", normalizedQuery, "replace");
    }, 350);

    return () => window.clearTimeout(timeout);
  }, [filters.q, query, setUrlFilter]);
  ...
  return (
    ...
    >
      ...
      >
        <Card className="gap-4 py-4 shadow-none">
          <CardContent className="grid gap-2 px-4 lg:grid-cols-[minmax(260px,1fr)_160px_220px_150px_auto]">
            ...
            <Select
              ...
              onValueChange={(value) => setUrlFilter("status", value)}
            >
            ...
            <Select
              value={filters.route}
              onValueChange={(value) => setUrlFilter("route", value)}
            >
            ...
            <Select
              value={filters.range}
              onValueChange={(value) =>
                setUrlFilter("range", value as LogTimeRange)
              }
            >
        ...
        <Card className="gap-0 py-0 shadow-none">
          <CardHeader className="border-b py-5">
            <div>
              <CardTitle id="logs-heading">Log events</CardTitle>
            ...
            <CardAction>
              <Badge variant="soft" color="neutral" size="24">
                <Database />
                Newest first
              </Badge>
            </CardAction>
          ...
          {events.length > 0 ? (
            ...
            <CardContent className="px-0">
              <Empty className="min-h-64 px-5 py-12">
                <EmptyMedia variant="icon">
                  {hasActiveFilters ? <Search /> : <Database />}
                </EmptyMedia>
                <EmptyHeader>
                  <EmptyTitle>
                    {hasActiveFilters ? "No matching events" : "No logs yet"}
                  </EmptyTitle>
                  <EmptyDescription>
                    {hasActiveFilters
                      ? "Try widening the time range or clearing the current filters."
                      : "No events have been received for this workspace. Create an API key and send your first request."}
                  </EmptyDescription>
                </EmptyHeader>
                <EmptyAction>
                  {hasActiveFilters ? (
      ...
      {selectedEvent ? (
        <LogInspector key={selectedEvent.id} event={selectedEvent} />
      ) : null}
    </Drawer>
  );
}
